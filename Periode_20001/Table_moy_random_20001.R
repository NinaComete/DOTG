...
# Graph periode 20001 transmission aléatoire moyenne des random ------
...
calculate_row_mean <- function(tables) {
  ...
  df_moyennes <- as.data.frame(do.call(rbind, mean_rows))
  
  # Récupérer les noms de colonnes de la première table
  col_names <- colnames(tables[[1]])
  
  return(list(df_moyennes, col_names))
}
...
df_moyennes <- mean_data[[1]]
col_names <- mean_data[[2]]

# Ajouter une rangée pour les noms de colonnes
colnames(df_moyennes) <- col_names

# Exporter la table moyenne en tant que fichier CSV
write.csv(df_moyennes, file = "table_moyenne_random_20001.csv", row.names = TRUE)
