---
title : "Moyennes des valeurs maximales, de précision moyenne des connaissances et de succès moyen des interactions, dans les populations d'agents sans génération (période = durée de vie = durée de la simulation = 200001 itérations)"
...
create_summary_table <- function() {
  ...
  for (url in urls) {
    ...
    # duree de la simulation avec periode = 200001
    if (grepl("200001", url)) {
      duree_simulation <- 200001
    }
    
    # Ajouter les informations aux vecteurs
    duree_simulation <- c(duree_simulation)
  ...
  tableau_200001_random <- data.frame(duree_simulation = duree_simulation,
  ...
  return(tableau_200001_random)
}

# Créer le tableau récapitulatif
tableau_200001_random <- create_summary_table()



# Tableau des moyennes par duree_simulation ----

# Regrouper les données par la variable "duree_simulation"
grouped_data <- tableau_200001_random %>%
  group_by(duree_simulation)
...
kable(mean_data, format = "html", align = "c", 
      col.names = c("Durée de la simulation", 
                    "Max précision moyenne (avg.genAccuracy)", 
                    "Max succès moyen (avg.genSrate)"),
      row.names = FALSE, 
      caption = "Moyennes des valeurs maximales, de précision moyenne des connaissances et de succès moyen des interactions, dans les populations d'agents sans génération (période = durée de vie = durée de la simulation = 200001 itérations)",
      booktabs = TRUE) %>%
...
kable(tableau_200001_random, format = "html", align = "c", 
      col.names = c("Durée de la simulation", 
                    "Max précision moyenne (avg.genAccuracy)", 
