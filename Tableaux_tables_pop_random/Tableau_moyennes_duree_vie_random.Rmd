---
title : "Moyennes des valeurs maximales, de précision moyenne des connaissances et de succès moyen des interactions, selon la durée de vie des agents (7501, 10001 ou 20001 itérations) dans les populations d'agents où la transmission est aléatoire (random) et effective (entre générations)"
...
# Tableau des moyennes par durée de vie ----

# Regrouper les données par la variable "duree_de_vie"
grouped_data <- tableau_durees_vies_random %>%
  group_by(duree_de_vie)
...
kable(mean_data, format = "html", align = "c", 
      col.names = c("Durée de vie moyenne des agents", 
                    "Moyenne des maximums de precision moyenne (avg.genAccuracy) atteinte", 
                    "Moyenne des maximums de succès moyen (avg.genSrate) \natteint"),
      row.names = FALSE, 
      #caption = "Tableau recapitulatif des resultats",
      booktabs = TRUE) %>%
  kable_styling(full_width = FALSE, font_size = 10)  # R?duire la taille de police ? 10 points
